...
{
    public class UpsertExchangeConfigCommandValidator : AbstractValidator<UpsertExchangeConfigCommand>
    {
        public UpsertExchangeConfigCommandValidator()
        {
            RuleFor(x => x.ExchangeName)
                .IsInEnum().WithMessage("Invalid exchange name. Supported exchanges: Binance, KuCoin");
