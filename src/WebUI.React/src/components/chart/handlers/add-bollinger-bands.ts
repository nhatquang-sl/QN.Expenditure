...
const addBollingerBands = (chart: IChartApi, rsiChart: IChartApi, candles: Kline[]) => {
    ...
    if (i > 20) {
      ...
      if (line) {
        ...
        if (!supportSet.has(line)) {
          ...
          support.setData(
            klines.map((k) => ({ time: k.openTime / 1000 + TZ_OFFSET_SECONDS, value: line } as LineData))
          );
  ...
  sma20Series.setData(klines.map((k) => ({ time: k.openTime / 1000 + TZ_OFFSET_SECONDS, value: k.sma20 } as LineData)));
  boluSeries.setData(klines.map((k) => ({ time: k.openTime / 1000 + TZ_OFFSET_SECONDS, value: k.bolu } as LineData)));
  boldSeries.setData(klines.map((k) => ({ time: k.openTime / 1000 + TZ_OFFSET_SECONDS, value: k.bold } as LineData)));
  ...
  rsiSeries.setData(
    klines.map((c: Kline) => {
      return {
        time: c.openTime / 1000 + TZ_OFFSET_SECONDS,
        value: c.rsi,
      } as HistogramData;
    })
  );
