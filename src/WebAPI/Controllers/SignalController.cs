...
namespace WebAPI.Controllers
{
    ...
    {
        public static readonly ActivitySource MyActivitySource = new(nameof(SignalsController));
        ...
        /// <param name="sortBy">
        ///     Sort field: createdAt (default), entryHitAfterMinutes, maxProfitHitAfterMinutes,
        ///     stopLossHitAfterMinutes
        /// </param>
        /// <param name="sortOrder">Sort direction: asc or desc (default: desc)</param>
        /// <returns>
        ///     Paginated list of signals with entryHitAfterMinutes, maxProfitHitAfterMinutes, and stopLossHitAfterMinutes
        ///     persisted fields
        /// </returns>
        /// <summary>
        ...
        {
            using var activity = MyActivitySource.StartActivity("GetStatistics", ActivityKind.Server);
            activity?.SetTag("path", Request.Path);
            return sender.Send(new GetSignalsStatisticsQuery(interval, signalType));
        }
        ...
        {
            using var activity = MyActivitySource.StartActivity("GetSignals", ActivityKind.Server);
            activity?.SetTag("sortBy", sortBy);
            return sender.Send(new GetSignalsQuery(from, to, interval, signalType, pageNumber, pageSize, sortBy,
                sortOrder));
        }
    }
}