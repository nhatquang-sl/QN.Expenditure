...
11. As a developer, I want the RSI+BB mean-reversion strategy to generate a BUY signal when RSI < 30 AND Close < BBLower, so that both conditions must be satisfied simultaneously.
12. As a developer, I want boundary conditions explicitly defined (`RSI < 30` not `RSI <= 30`, `Close < BBLower` not `Close <= BBLower`), so that the strategy behavior at exact thresholds is unambiguous.
13. As a developer, I want a directional scoring model applied after signal detection, so that the strength of the signal is quantified for downstream consumers.
14. As a developer, I want RSI scoring to use tiers (RSI < 30 = +30, RSI < 25 = +45, with the stronger tier replacing the weaker), so that extreme oversold conditions score higher without double-counting.
15. As a developer, I want BB Width below 0.02 to contribute +10 to the score as a configurable threshold, so that low-volatility squeeze conditions increase signal confidence.
16. As a developer, I want RSI Slope direction to contribute ±15 to the score, so that a rising RSI on a BUY signal adds confidence while a falling RSI reduces it.
17. As a developer, I want a `Signal` struct with `Side`, `Score`, and `Reasons []string`, so that downstream consumers know both the direction and the rationale for the signal.
18. As a developer, I want `Reasons` to contain named constants (e.g. `RSI_EXTREME_OVERSOLD`, `PRICE_BELOW_BB_LOWER`), so that decision context is human-readable and machine-parseable.
19. As a developer, I want a `CandidateTrade` struct that captures the full decision snapshot (symbol, timeframe, side, entry price, strategy name, strategy version, all indicator values, score, reasons, idempotency key), so that any future consumer can reconstruct why the trade was proposed.
20. As a developer, I want `EntryPrice` set to the close price of the triggering candle, so that the value is deterministic and requires no additional API call.
21. As a developer, I want a `CandidateTradeFactory` that generates a UUID and idempotency key from `Symbol:Timeframe:CloseTime:StrategyName:StrategyVersion`, so that duplicate detection is built into the creation contract.
22. As a developer, I want the pipeline to only evaluate the strategy on closed candles, so that signal repainting and duplicate signals from in-progress candles are prevented.
23. As a developer, I want `CandidateTrade` records persisted to a `trading_bot` PostgreSQL database with a `UNIQUE` constraint on the idempotency key, so that duplicate signals are prevented across process restarts.
24. As a developer, I want the `INSERT` to use `ON CONFLICT (idempotency_key) DO NOTHING`, so that idempotent re-processing is safe without error-handling overhead.
25. As a developer, I want a structured `slog` log event emitted only when a new (non-duplicate) `CandidateTrade` is created, so that observability is accurate without excessive noise.
26. As a developer, I want the polling loop to run every 5 minutes, fetching 1000 closed candles per symbol per timeframe from KuCoin, so that the pipeline is stateless and RSI is always sufficiently warm.
27. As a developer, I want both `1hour` and `4hour` timeframes active from day one, so that the pipeline produces signals across multiple market contexts.
28. As a developer, I want the list of monitored symbols to be configurable (default: `["BTCUSDT"]`), so that adding new symbols requires only a config change.
29. As a developer, I want all indicator configuration (RSI period, BB period, BB multiplier, RSI slope period) to be part of a versioned config struct, so that backtests are reproducible by replaying the same config.
30. As a developer, I want the RSI+BB strategy parameters (oversold thresholds, BB width threshold) to be part of a versioned `RSIBBConfig`, so that strategy version changes are traceable.
31. As a developer, I want every `CandidateTrade` to store `StrategyName` and `StrategyVersion` (e.g. `RSI_BB_MEAN_REVERSION`, `1.0.0`), so that backtests can attribute each trade to an exact strategy version.
32. As a developer, I want the `Side` type to support both `BUY` and `SELL` from day one, so that adding SELL signal generation in a future sprint requires no domain model changes.
33. As a developer, I want indicator values validated against TradingView reference data within ±0.0001 tolerance, so that the implementation is provably correct before strategy logic is built on top.
34. As a developer, I want an integration test that runs the full pipeline with fixture candles and asserts the resulting `CandidateTrade` fields, so that regressions in the end-to-end path are caught automatically.
35. As a developer, I want a determinism test that runs the same fixture candles through the pipeline twice and asserts that all meaningful output fields are identical, so that hidden mutable state or time-dependency bugs are caught.
36. As a developer, I want `calculateRSISeries()` to return the full RSI time series as `[]float64` (one value per candle from index `RSIPeriod`), so that divergence detection can scan historical RSI peaks and troughs without re-computing RSI.
37. As a developer, I want `IndicatorSnapshot` to include a `DivergenceType` field (None / Bearish / Bullish), so that strategies can consume pre-computed divergence state without accessing raw candle history directly.
38. As a developer, I want divergence detection parameters (`PeakThreshold=68`, `TroughThreshold=32`, `LookbackCandles=20`) to live in `IndicatorConfig.Divergence`, so that detection behaviour is versioned alongside other indicator configuration and matches TradingView defaults.
39. As a developer, I want `calculateDivergence()` to identify a bearish divergence when the current candle is an RSI peak (RSI > 68, strict) AND a previous peak within `LookbackCandles` has higher RSI but lower price high, so that price higher-high / RSI lower-high patterns are detected.
40. As a developer, I want `calculateDivergence()` to identify a bullish divergence when the current candle is an RSI trough (RSI < 32, strict) AND a previous trough within `LookbackCandles` has lower RSI but higher price low, so that price lower-low / RSI higher-low patterns are detected.
41. As a developer, I want the RSI Divergence strategy to generate a BUY signal on `DivergenceBullish` and a SELL signal on `DivergenceBearish`, so that both bullish and bearish momentum-divergence patterns produce actionable candidates.
42. As a developer, I want divergence scoring to use a base of +50 with RSI Slope contributing ±15 (confirming direction +15, counter direction -15), so that signal strength reflects both the divergence structure and momentum confirmation.
43. As a developer, I want `RSI_BULLISH_DIVERGENCE` and `RSI_BEARISH_DIVERGENCE` as named reason constants, so that downstream consumers can identify divergence-sourced signals programmatically.
44. As a developer, I want `calculateDivergence()` edge cases tested with synthetic RSI series (no divergence, bearish pattern, bullish pattern, prior peak/trough outside lookback window, RSI exactly at threshold), so that divergence detection correctness is independently verifiable without relying on the fixture candle set containing a divergence.
45. As a developer, I want a separate Seam 3 integration test for `DivergenceStrategy`, wiring `FileCandleRepository → IndicatorEngine → DivergenceStrategy → CandidateTradeFactory`, so that its end-to-end pipeline is verified independently of `RSIBBStrategy`.
46. As a developer, I want the RSI Divergence strategy versioned as `StrategyName = "RSI_DIVERGENCE"`, `StrategyVersion = "1.0.0"`, so that its `CandidateTrade` records are attributable to this exact strategy implementation.
...
- **Directional scoring (RSI+BB BUY side)**:
  - RSI < 30: +30; RSI < 25: +45 (replaces, not additive)
  - Close < BBLower: +30
  - RSI Slope > 0: +15
  - BBWidth < `BBWidthThreshold` (default 0.02): +10
  - `BBWidthThreshold` is a scoring-only feature, not a gate condition.
...
- **Idempotency key format**: `BTCUSDT:1hour:2026-09-16T10:00:00Z:RSI_BB_MEAN_REVERSION:1.0.0` (canonical symbol, timeframe as configured string, candle `CloseTime` in RFC3339 UTC, strategy name, strategy version).
...
- **Strategy versioning**: Two strategies ship in this sprint: `RSI_BB_MEAN_REVERSION / 1.0.0` and `RSI_DIVERGENCE / 1.0.0`. Version bumps when strategy logic or scoring model changes. Never reuse a version string for a changed strategy.
...
**Seam 2 — `RSIBBStrategy.Evaluate(ctx MarketContext) (*Signal, error)`**
- Tests construct `MarketContext` directly with hardcoded `IndicatorSnapshot` values — no candles, no engine.
- Five required cases: no signal; RSI oversold only; price below BBLower only; both conditions met (BUY); boundary values (`RSI == 30` no signal, `RSI == 29.9999` signal; `Close == BBLower` no signal, `Close == BBLower - 0.01` signal).
- Also verify scoring: RSI < 25 produces +45 (not +75), RSI Slope > 0 adds +15, BBWidth below threshold adds +10.

**Seam 2b — `DivergenceStrategy.Evaluate(ctx MarketContext) (*Signal, error)`**
...
**Seam 3 — Full pipeline integration (RSIBBStrategy, via fake `CandleRepository`)**
- Uses a `FileCandleRepository` that loads fixture candles from disk. No real KuCoin call. No real database.
- Wires: `FileCandleRepository` → `IndicatorEngine` → `RSIBBStrategy` → `CandidateTradeFactory` → `CandidateTrade`.
- Asserts: correct `Side`, `StrategyName`, `StrategyVersion`, `Indicators.RSI` within tolerance, expected `Reasons`.
- Determinism sub-test: run same fixture twice, assert `Side`, `EntryPrice`, `StrategyName`, `StrategyVersion`, `Score`, `Indicators`, `Reasons` are identical. Exclude `Id` and `CreatedAt`.

**Seam 3b — Full pipeline integration (DivergenceStrategy, via fake `CandleRepository`)**
- Separate test function from Seam 3.
...
- Adaptive BB Width threshold based on historical percentiles
...
### Slice 6 — RSI+BB strategy (BUY signal)

Deliverable: `RSIBBStrategy.Evaluate()` returns a `*Signal` with correct `Side`, `Score`, and `Reasons` when both gate conditions are met; returns `nil` otherwise.

Done when:
- Gate: `RSI < 30 AND Close < BBLower` → BUY signal
- Scoring: RSI tiers (+30/+45), Close < BBLower (+30), RSI Slope (+15), BBWidth (+10)
- All five strategy unit test cases pass (Seam 2)
- Boundary conditions (`RSI == 30`, `Close == BBLower`) explicitly tested

---

### Slice 7 — CandidateTrade + factory + in-memory idempotency
...
- Full Seam 3 integration test passes (RSIBBStrategy)
- Full Seam 3b integration test passes (DivergenceStrategy)
- Determinism sub-test passes (same fixture → same meaningful fields twice)

---

### Slice 8 — PostgreSQL persistence
...
### Slice 10 — Polling runner + binary
...
- The BB Width threshold of 0.02 is intentionally conservative for the MVP. It should later be replaced by a historical percentile measure (e.g. current BBWidth < 20th percentile of last N candles) to be robust across symbols and timeframes.
...
- The RSI Divergence strategy (`RSI_DIVERGENCE / 1.0.0`) ships alongside RSI+BB in this sprint. Its scoring model (base +50, slope ±15, range 35–65) is intentionally minimal. Scoring tiers should only be added after backtest data is available to justify them.
